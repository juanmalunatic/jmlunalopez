...
class AdminSupport
{
    ...
    private function handleGET()
    {
        ...
        <div id="wpsynchro-support" class="wrap wpsynchro">
            <h2 class="wpsynchro-page-header-title"><img src="<?= $commonfunctions->getAssetUrl("icon.png") ?>" width="35" height="35" />WP Synchro <?= WPSYNCHRO_VERSION ?> <?php echo(\WPSynchro\CommonFunctions::isPremiumVersion() ? 'PRO' : 'FREE'); ?> - <?php _e('Support', 'wpsynchro'); ?></h2>
            ...
            if (\WPSynchro\CommonFunctions::isPremiumVersion() && $licensing->verifyLicense()) {
                ...
                <ul>
                    ...
                    <li> - <?php _e('Log file from migration (found in menu "Logs")', 'wpsynchro'); ?></li>
                </ul>
            ...
            <p><?php _e('Contains debug information about the migration, such as configuration and file locations. No personal information is included.', 'wpsynchro'); ?></p>
            ...
            <p><?php _e('Delete all data related to WP Synchro, in database and files. Can be used to clean up after WP Synchro if needed.', 'wpsynchro'); ?><br><?php _e('Does not reset access key and license key setup, but removes data like log files and migrations.', 'wpsynchro'); ?></p>
