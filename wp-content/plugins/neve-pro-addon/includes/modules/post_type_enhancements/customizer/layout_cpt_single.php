...
use Neve_Pro\Traits\Core as TraitsCore;
...
class Layout_CPT_Single extends Base_Customizer {
	...
	public function add_controls() {
		...
		$this->is_new_skin         = neve_is_new_skin();
		...
		$this->add_boxed_layout_controls(
			...
			[
				...
				'active_callback'         => function () {
					return $this->model->is_custom_layout_enabled()
						&& $this->element_is_enabled( 'comments' )
						&& Mods::get( 'neve_' . $this->model->get_type() . '_comments_form_boxed_layout', neve_is_new_skin() );
				},
			]
		);
	}
	...
	private function headings() {
		$related_no_controls = $this->is_new_skin ? ( $this->supports_boxed ? 10 : 6 ) : 4;
	...
	public function related_posts() {
		...
		$this->add_control(
			new Control(
				'neve_related_' . $this->model->get_type() . '_columns',
				[
					'sanitize_callback' => 'neve_sanitize_range_value',
					'default'           => 3,
				],
